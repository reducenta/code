...
if ($shipment !== null) {
    ...
    foreach (OrderHelper::calcDeliveries($shipment, $availableDeliveries) as $deliveryID => $calculationResult) {
        ...
        //$arDelivery['NAME'] = $obDelivery->getNameWithParent();
        $arDelivery['NAME'] = $obDelivery->getName();
        $arDelivery['DESC'] = $obDelivery->getDescription();
        $arDelivery['CHECKED'] = $checkedDeliveryId === $obDelivery->getId();
        $arDelivery['PRICE'] = $calculationResult->getPrice();
        if($calculationResult->getDeliveryPrice() == 0){
            $arDelivery['PRICE_DISPLAY'] = 'Бесплатно';
        }else{
...
if(!$checked){
    $arResult['DELIVERY_LIST'][array_key_first($arResult['DELIVERY_LIST'])]['CHECKED'] = 1;
}
...
foreach ($availablePaySystem as $paySystem) {
    ...
    if($arPaySystem['ID'] == $checkedPaySystemId){
        $arPaySystem['CHECKED'] = 1;
    }else{
        $arPaySystem['CHECKED'] = null;
    }
...
$checked = false;
foreach($arResult['PAY_SYSTEM_LIST'] as $item){
    if($item['CHECKED'] == 1){
        $checked = true;
    }
}
...
if(!$checked){
    $arResult['PAY_SYSTEM_LIST'][array_key_first($arResult['PAY_SYSTEM_LIST'])]['CHECKED'] = 1;
}
